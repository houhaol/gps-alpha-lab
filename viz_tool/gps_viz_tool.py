...
def open_and_populate_data():
    ...
    world_df["rel timestamp [s]"] = world_df["timestamp [ns]"] - world_df["timestamp [ns]"].min()
    ...
    return world_gaze_gps_imu_df, world_gps_imu_df, gps_imu_df, world_df, events_df, gps_df
...
def reverse_geocode_events(world_gaze_gps_imu_df, events_df):
    ...
    for idx, row in events_df.iterrows():
        idx = world_gaze_gps_imu_df.index.get_indexer([row["timestamp"]], method="nearest")[0]
        world_row = world_gaze_gps_imu_df.iloc[idx]
    ...
    geocoded_events_df = pd.DataFrame(
        {
            ...
            "size": [20 for event in event_gps_list],
        }
    )
...
def calculate_arrow_latlon_coords(lat, lon, heading, scale=0.0002):
    """
...
def create_base_map_figure(world_gaze_gps_imu_df, world_df, geocoded_events_df):
    ...
    fig = px.line_map(world_gaze_gps_imu_df, lat="latitude", lon="longitude", zoom=16, height=500)
    ...
    idx = world_gaze_gps_imu_df.index.get_indexer([target_timestamp], method="nearest")[0]
    ...
    heading_arrow_lon, heading_arrow_lat = calculate_arrow_latlon_coords(initial_lat, initial_lon, inital_heading)
    heading_arrow_df = pd.DataFrame(
        {"lat": [initial_lat, heading_arrow_lat], "lon": [initial_lon, heading_arrow_lon]}
    )
    ...
    gaze_arrow_lon, gaze_arrow_lat = calculate_arrow_latlon_coords(initial_lat, initial_lon, initial_gaze_azi)
    ...
    wearer_marker = go.Scattermap(
        ...
        lon=[0],
        mode="markers",
        marker=dict(size=12, color="black", opacity=1.0),
...
def find_neon_video_path(neon_folder_path):
    ...
    if neon_scene_filename is None:
        print("Error: No Neon scene video in the 'assets' subdirectory. Please read the instructions.", file=sys.stderr)
        sys.exit(1)
    else:
        neon_scene_path = os.path.join("./assets", filename)
        return neon_scene_path


# load up all data, prepare fig, find neon scene video
world_gaze_gps_imu_df, world_gps_imu_df, gps_imu_df, world_df, events_df, gps_df = open_and_populate_data()
geocoded_events_df, event_gps_list = reverse_geocode_events(world_gaze_gps_imu_df, events_df)
...
def map_update_on_currentTime(currentTime, current_fig):
    ...
    if idx < len(gps_imu_df):
        ...
        new_x = new_lon + 0.0002 * np.cos(np.radians(heading))
        new_y = new_lat + 0.0002 * np.sin(np.radians(heading))
        ...
        # Compute new arrow coordinates
        new_x = new_lon + 0.0002 * np.cos(np.radians(gaze_azi))
        new_y = new_lat + 0.0002 * np.sin(np.radians(gaze_azi))
...
def update_video_on_event_selection(start_event, end_event):
    if start_event is not None and end_event is not None:
        ...
        start_timestamp = event_gps_list[start_event]["timestamp"] - world_gps_imu_df.index.min()
        start_timestamp = start_timestamp.total_seconds()

        # Get the end event's timestamp and convert it to seconds.
        end_timestamp = event_gps_list[end_event]["timestamp"] - world_gps_imu_df.index.min()
...
def update_map_on_event_selection(start_event, end_event, current_fig):
    if start_event is not None and end_event is not None:
        start_timestamp = event_gps_list[start_event]["timestamp"]
        end_timestamp = event_gps_list[end_event]["timestamp"]
        
        subset_df = world_gaze_gps_imu_df[(world_gaze_gps_imu_df.index >= start_timestamp) & (world_gaze_gps_imu_df.index <= end_timestamp)]
...
def seek_video(selected_gps_event, clickData):
    ...
    if clickData:
        ...
        if point_index is not None:
            ...
            timestamp = world_gps_imu_df.index[point_index] - world_gps_imu_df.index.min()
